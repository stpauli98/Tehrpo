...
import type { ProposalData } from "@/lib/claude/protokol"

/** UI-only tip poruke u toku (nije dio stream protokola). */
...
function ZapisnikProposal({ p }: { p: ProposalData }) {
  ...
  // Jedan kanal feedbacka (S2): greška ide ISKLJUČIVO u toast. Inline ostaje samo
  // kratak vizuelni indikator uspjeha („Snimljeno"), a toast nosi vlastitu rečenicu —
  // ista poruka se nikad ne pojavi dvaput.
  useAkcijaToast(state, { uspjeh: t("uspjeh"), greska: t("greska") })
  ...
  return (
    <div className="mt-2 rounded-xl bg-card p-3 ring-1 ring-foreground/10" data-testid="zapisnik-proposal">
      ...
      {mozeUrediti && (
        <form action={action} className="mt-2">
          ...
          {/* disable poslije uspjeha — svaki dodatni klik pravi novi dokument (S8.8) */}
          <Button type="submit" disabled={pending || snimljeno} data-testid="snimi-zapisnik">
            {pending ? t("snimam") : t("snimi")}
          </Button>
          {snimljeno && (
            <span className="ml-2 inline-flex items-center gap-1 text-xs text-success" data-testid="zapisnik-snimljen">
...
export function ChatMessage({ poruka }: { poruka: UiPoruka }) {
  ...
  return (
    <div className={isUser ? "flex justify-end" : "flex justify-start"} data-testid={`msg-${poruka.role}`}>
      <div className={`max-w-[80%] rounded-xl px-4 py-2 text-sm ${isUser ? "bg-brand text-white" : "bg-brand-light text-foreground"}`}>
        ...
        {isUser ? (
          ...
          <p className="text-muted-foreground">{t("pisanje")}</p>
        )}
