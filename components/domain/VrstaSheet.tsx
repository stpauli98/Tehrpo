...
export function VrstaSheet({ vrsta }: { vrsta: Vrsta }) {
  ...
  // pod-komponenti, pa remount briše zaostalu poruku greške i vraća polja na svježe
  // `vrsta.*` vrijednosti (zamjenjuje raniji `key={vrsta.id}` na formi).
  ...
  return (
    ...
    >
      ...
      <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto" data-testid="vrsta-sheet">
        ...
        <VrstaForma key={`${vrsta.id}-${instanca}`} vrsta={vrsta} onGotovo={zatvori} />
...
function VrstaForma({ vrsta, onGotovo }: { vrsta: Vrsta; onGotovo: () => void }) {
  const router = useRouter()
  const t = useTranslations("postavke.vrstaSheet")
  const tc = useTranslations("common")
  ...
  useEffect(() => {
    if (submitted.current && !pending && state.ok) { submitted.current = false; onGotovo(); router.refresh() }
