...
test.describe("Podsjetnici — combobox primalaca (kontakti + ad-hoc)", () => {
  test("izabere kontakt, doda ad-hoc mejl, oba perzistiraju, pa se uklone", async ({ page }) => {
    ...
    try {
      ...
      // PR #83 je dodao sekciju LOKACIJA u kontakt sheet. Tada je klijent bez lokacija
      // imao SAMO granu „Nova lokacija" sa obaveznim nazivom, pa je ovaj korak morao
      // popuniti `kontakt-nova-lokacija-naziv` (inače HTML5 validacija blokira submit).
      // Popravka 5643856 (audit grana) je vratila kontakt firme kao izbor i učinila ga
      // PODRAZUMIJEVANIM — polje za naziv nove lokacije se sada ne renderuje dok se
      // grana „Nova lokacija" izričito ne izabere, pa bi stari `fill` visio do timeout-a.
      // Ovdje je kontakt firme i semantički tačan: primalac podsjetnika za sve lokacije.
      // Tvrdnja umjesto tihog oslanjanja na default — čuva obje popravke (isti obrazac
      // kao 38-yoink-batch).
      ...
      //
      // Namjerno BEZ retry-a i BEZ pressSequentially zaobilaznice (obje su ranije bile
      // ovdje i obje su uklonjene): klik na opciju mora raditi iz prvog puta u OBA
      // browsera. Na webkitu nije radio jer je PrimaociCombobox zatvarao listu na blur
      // prije nego onClick stigne (Safari ne fokusira <button> na klik, pa je
      // relatedTarget bio null) — popravljeno u komponenti sa onMouseDown +
      // preventDefault. Ovaj korak je čuvar te popravke: retry bi ga učinio zelenim i sa
      // vraćenim bugom, a korisnici na Safariju ne bi mogli izabrati primaoca
      // podsjetnika.
      await input.click()
