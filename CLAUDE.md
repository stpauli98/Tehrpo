...
- **Merge to `main` = production deploy.** Vercel git-integration auto-deploys `main` to **three** Production projects at once (there is no deploy GitHub Action, and scheduling lives in `vercel.json`, not in CI). The repo *does* have one workflow — `.github/workflows/gate.yml`, added after 2026-07-20: it runs on PRs to `main` and on `integracija/**` pushes (typecheck, lint, unit, build, `provjeri:integraciju`) against **DEMO** env values only. It gates, it does not deploy. Vercel project names are **inverted from role**: `tehpro-demo.nextpixel.dev` (Vercel project `tehpro-demo`) is PROD → `fqtqkehjidkzeasiegnq`; `de.nextpixel.dev` + `demo.nextpixel.dev` are DEMO → `mtwwotmwrasozmcgqwhc`. Branch push = Preview deploy. Don't push `main` casually.
...
- **Deploy:** Vercel, region `dub1` (`vercel.json`). The reminder cron is wired **only** through `vercel.json` (`/api/cron/reminders`, **two runs a day** — `0 9` and `0 13` UTC, i.e. 10:00/14:00 Vienna in winter, 11:00/15:00 in summer), one schedule pair per Vercel project. The route itself decides whether to act: `podsjetnici_aktivni`, `vrijeme_slanja_sat`, and the daily `zadnje_slanje_datum` marker (pre-due only). The gate (`sat >= vrijeme_slanja_sat`) doesn't retry hourly — it picks **which** of the two daily runs actually sends, based on the configured slot ("morning" vs "afternoon"); post-due recovery of a stale claim (15 min) still matters since the two runs are hours apart. This cron-schedule ↔ settings link lives in `lib/reminders/rasporedSlanja.ts` and is guarded by its test file — see that module before touching either side. A `.github/workflows/reminders.yml` used to exist as a second scheduler but **never worked**: its four secrets were never set, so `curl` got an empty URL and `|| true` made every run report success. It was removed on 2026-07-20 rather than left to lie.
